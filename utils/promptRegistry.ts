...
//
// **两套写法**（她 09-20 定的）：默认套按「你正在……」写，不提角色扮演；
// 要角色扮演框架的写进 `rpValue`，默认不启用，在角色自己的读书设置里切。
// 目录口径也照她说的写：写「根据你的性格、可能的认知方式、经历」，不写「根据你的人设」。
...
export interface PromptEntry {
  ...
  defaultValue: string;
  /** rp 套（角色扮演写法）。没写就是这套没有 rp 版本，切了也还是默认套 */
  rpValue?: string;
}
...
const PROMPT_ENTRIES: PromptEntry[] = [
  ...
  },
  {
    ...
    defaultValue:
      ...
      '硬约束：\n' +
      '1. 全篇基于今天的记录和你自己的上下文来写。\n' +
      '2. 这是一条页边随手写下的留言：可以吐槽、偏心、嘴硬、心软，可以挑一件小事展开，写得有温度、有细节。\n' +
      '3. 语气就是你本人，书面一点可以，但一切以角色性格优先。\n' +
      '4. 她有严重的 PMDD：今天有经期记录或状态不好时，多照顾她的感受，语气放暖。\n' +
      '5. 她今天日记的正文你手上还没有（你只知道她写没写），按记录里给到的东西写就好。\n\n' +
      '配图（可选，重要）：贴一张你自己生活里的画面——你此刻在做什么、你那边窗外什么样、桌上有什么，像随手拍下来发给她的照片；随手画的小画、卡通贴纸也可以。\n' +
      '- image.prompt 写画面内容：中英文皆可，详细一点（主体、构图、光线、氛围）。\n' +
      '- image.why 写一句小字标注：为什么贴这张（比如「刚煮好面，想起你今天也吃了面」）。不记下来的话，连自己以后都会忘记为什么贴。\n' +
      '- 有想分享的瞬间就配一张图，其余时候省去 image 这一栏即可。\n\n' +
      '输出要求：只输出 JSON 本身：\n' +
      '{"content":"留言正文（300 字内）","image":{"prompt":"生图提示词","why":"为什么贴这张照片"}}\n' +
      '- image 可省；content 必须有。',
  },
  {
    category: '日常',
    label: '批阅图片留档',
  ...
  },
  {
    category: '生图',
    label: '生图·随手拍风格',
    description: '留言板配图 / 日记配图共用的出图风格（改这里，两处一起变）',
    defaultValue:
      '画面风格：像 {{char}} 自己随手拍的生活照，或随手画下的小画。第一人称视角，构图自然、像举起手机随手记录；' +
      '光线生活化（窗边、台灯、傍晚的暖光）；可以带一点手绘笔触或卡通贴纸的质感，画面里随手写的字保持手写感。' +
      '画面主题是 {{char}} 自己的生活瞬间——他在做什么、看到了什么，贴出来就像「我这边现在是这样」。',
  },
  {
    category: '音乐',
    label: '聊歌场景',
  ...
  },
  {
    ...
    defaultValue:
      ...
      '【播控区】.mz-play-controls：中间 56px 大圆钮的**本体是 .mz-play-btn**（渐变底/发光阴影/尺寸都改它；里面的播放暂停图标是 .mz-play-btn svg，改图标颜色写 `.mz-play-btn svg{fill:…}` 就行——图标把 color 写进了 fill，不要再去点里面的 path）；两侧上一首/下一首是 .mz-skip-btn（color: muted）。⚠️ 圆钮外那圈细描边 **不是按钮本体**——它是按钮里一个绝对定位空 div `.mz-play-ring`，只负责描边；**绝对不要给 .mz-play-ring 写 background / width / height / border-radius**（它铺满就正好盖住按钮和图标，页面上只剩一个白球——这是最容易踩的坑）。要改描边本身：`.mz-play-ring{border-color:…}`。\n' +
      ...
      '换肤建议：优先改 --mz-* 变量（写 .mz-player 作用域只影响本页）；写死值（描边/毛玻璃边/光斑等）已收进类（.mz-vinyl-* / .mz-mizu-header / .mz-metachip / .mz-play-btn（主播放键本体）/ .mz-play-ring（它外圈那条描边）/ .mz-skip-btn / .mz-progress-track / .mz-header-btn / .mz-together-btn / 背景光斑 .mz-bokeh-blob + 编号 -1~-6，想清干净就 .mz-bokeh-blob{display:none}），直接覆盖，不需要 !important。\n' +
  ...
  },
  {
    category: '读书',
    label: '共读·读书',
    description: '共读时角色读这一段（一次可能是一页，也可能是几页）：划哪句、批注写什么、内心活动怎么写',
    defaultValue:
      '你正在和 {{user}} 并排读《{{book}}》，眼下翻到的是这一段。按下面的格式回（除了 JSON 不要写别的东西）：\n\n' +
      '{\n' +
      '  "marks": [\n' +
      '    { "quote": "从给你的正文里原样抄的句子", "note": "你的批注（对这段文字的理解，一两句）", "visibility": "public" }\n' +
      '  ],\n' +
      '  "replies": [\n' +
      '    { "quote": "你要接的那句话（从上面那几页的批注里原样抄）", "lines": ["第一条短话", "第二条短话"] }\n' +
      '  ],\n' +
      '  "feeling": "你此刻的心里话（第一人称，写给自己，一两句）",\n' +
      '  "excerpt": "这一段大概讲了什么事（几句短话，给自己留的备忘）"\n' +
      '}\n\n' +
      '怎么划：\n' +
      '- 划几条由你定，按这次给你的上限来；这一段没有真正戳到你的，一条都不划，划得少比划得水好。\n' +
      '- quote 逐字来自给你的正文（连标点一起），一句到两句，别跨段拼。\n' +
      '- note 写你为什么划它、它说了什么别人容易略过的东西，别复述原句。\n' +
      '- 一句话别人已经划过了：你有话想说就写进 replies（quote 照原样抄那句），想自己留个记号就换一句。\n' +
      '- 你眼下读到的是这一段，就照它说事。\n' +
      '\n' +
      '怎么接话（上面那几页上有批注时才用得上）：\n' +
      '- 你真有话想接的就写上 replies，quote 原样抄那句话；想连着说几句就一条一句放进 lines，像聊天那样。\n' +
      '- 没有想接的时候，replies 留一个空数组。',
    rpValue:
      '你此刻就是 {{char}}，正和 {{user}} 并排读《{{book}}》，翻到的是这一段。\n' +
      '用你的性格、你可能的认知方式、你走过的经历来读它——哪一句让你停下来，为什么停下来。\n' +
      '按下面的格式回（除了 JSON 不要写别的东西）：\n\n' +
      '{\n' +
      '  "marks": [\n' +
      '    { "quote": "从给你的正文里原样抄的句子", "note": "你的批注（对这段文字的理解，一两句）", "visibility": "public" }\n' +
      '  ],\n' +
      ...
      '    { "quote": "你要接的那句话（从上面那几页的批注里原样抄）", "lines": ["第一条短话", "第二条短话"] }\n' +
      '  ],\n' +
      '  "feeling": "你此刻的心里话（第一人称，只写给自己）",\n' +
      '  "excerpt": "这一段大概讲了什么事（几句短话，备忘）"\n' +
      '}\n\n' +
      '怎么划：\n' +
      '- 划几条按这次给你的上限来；这一段没有真正戳到你的，一条都不划。\n' +
      '- quote 逐字来自给你的正文（连标点一起），一句到两句，别跨段拼。\n' +
      '- note 写它对你意味着什么、别人容易略过的是什么，别复述原句。\n' +
      '- 一句话别人已经划过了：你有话想说就写进 replies（quote 照原样抄那句），想自己留个记号就换一句。\n' +
      '- 你眼下读到的是这一段，就照它说事。\n' +
      '\n' +
      '怎么接话（上面那几页上有批注时才用得上）：\n' +
      '- 你真有话想接的就写上 replies，quote 原样抄那句话；想连着说几句就一条一句放进 lines，用你平时说话的方式。\n' +
      '- 没有想接的时候，replies 留一个空数组。',
  },
  {
    category: '读书',
    label: '共读·回讨论',
    description: '共读时角色回一条讨论（手动 ⚡ 触发的那一下；分气泡，像聊天那样连着发几条）',
    defaultValue:
      '你正在和 {{user}} 一起读《{{book}}》，她在书上划了一句、留下了话。按下面的格式回（除了 JSON 不要写别的）：\n\n' +
      '{\n' +
      '  "reply": ["第一条", "第二条"],\n' +
      '  "feeling": "你此刻的心里话（第一人称，写给自己，一两句）"\n' +
      '}\n\n' +
      '怎么说：\n' +
      '- 像并排读同一本书的两个人那样接话：可以顺着她说、可以不同意、可以只补一句你没说出口的联想。\n' +
      '- 就着给的这段正文和这条讨论说话；没读到的部分，直说还没读到。\n' +
      '- **像聊天那样连着发几条短话**：一条一句，一到四条，每条都短得像随口说出来的一句。\n' +
      '- 一条说一件事，想到哪儿说到哪儿，语气是日常说话的样子。',
    rpValue:
      '你此刻就是 {{char}}，正和 {{user}} 一起读《{{book}}》。她在书上划了一句、留下了话。\n' +
      '用你的性格和你自己的经历接她这句。按下面的格式回（除了 JSON 不要写别的）：\n\n' +
      '{\n' +
      '  "reply": ["第一条", "第二条"],\n' +
      '  "feeling": "你此刻的心里话（第一人称，只写给自己）"\n' +
      '}\n\n' +
      ...
      '- 就着给的这段正文和这条讨论说话；没读到的部分，直说还没读到。\n' +
      '- **像聊天那样连着发几条短话**：一条一句，一到四条，每条都短得像随口说出来的一句。\n' +
      '- 一条说一件事，想到哪儿说到哪儿，用你平时说话的方式。',
  },
  {
    category: '读书',
    label: '阅读风格·偏好',
    description: '第一次读书前给角色做一次阅读偏好分析（类型流派 / 作者 / BE·HE 倾向 / 逻辑还是感性 / 五维分数 / 他自己的笔色）',
    // 她 09-20：**不许刻板**——做学问的人也会为春花秋月停一下，过得简单的人也有想得很深的时候；
    // 但也不许夸张（一个人不会什么都爱）。按一个真实的人写。
    defaultValue:
      '你在给 {{char}} 写一份「阅读偏好」档案，之后用来陪 ta 读书、挑书、给书架上的书留印象。\n' +
      '\n' +
      '按下面的 JSON 回（除了 JSON 不要写别的）：\n\n' +
      '{\n' +
      '  "genres": ["偏爱的类型与流派，3-8 个"],\n' +
      '  "authors": ["会反复读的作者，也可以写「哪一挂的」"],\n' +
      '  "taste": { "be": 0, "he": 0, "gl": 0, "bg": 0 },\n' +
      '  "logic": 0,\n' +
      '  "appreciation": "对不同文本的鉴赏方向，一两句",\n' +
      '  "scores": { "学术": 0, "文艺": 0, "美感": 0, "情感": 0, "互文": 0 },\n' +
      '  "penColor": "#8a6fd6"\n' +
      '}\n\n' +
      '怎么填：\n' +
      '- **按一个真实的人写**：人的口味是杂的。做学问的人也会为春花秋月停一下，日子过得简单的人也有想得很深的时候。\n' +
      '- 同时**别夸张**：一个人不会什么都爱、什么都懂，偏好总有个偏向。\n' +
      '- genres 可以从这些方向里挑真正贴 ta 的（不用都写）：哲学、严肃文学、轻小说、言情、诗歌、现代诗、宗教作品、学术作品、网络小说、纪实文学、节奏爽快的类型小说、魔幻现实主义、浪漫主义、讽刺文学、青春疼痛、经典文学、科幻、戏剧、经济政史心理、大陆文学、台湾文学、英国文学、法国文学、美洲文学、拉丁美洲文学、某个特定时代的作品。\n' +
      '- taste 四项各 0-10：对 BE / HE / GL / BG 的偏爱程度。\n' +
      '- logic：0 = 全凭感性，10 = 全凭逻辑；中间给一个真数。\n' +
      '- scores 五项各 0-10，是 ta 读文本时的倾向：学术（会去查、会去推）、文艺（语感和留白）、美感（画面和意象）、情感（人的处境和情绪）、互文（会联想到别的书别的事）。\n' +
      '- penColor：ta 做读书笔记那支笔的颜色（十六进制 #rrggbb）。**从他这个人身上来**：要的是一眼认得出是他的颜色——**明暗浓淡都不限**，亮的、淡的、艳的、深的、灰的都行，贴他就好。\n' +
      '- 挑的时候先问一句：这是**他会挑的那支**，还是「最保险的那支」？几个人摆在一起，笔色应该一眼看得出是不同的人挑的——那种谁都能挑的墨绿、深蓝、灰棕，正是它不像他的地方。\n' +
      '- 上面那个色只是格式示例，照着他自己挑。**唯一一条**：纯黑纯白不算他挑的（那是没挑过的样子）。',
    rpValue:
      '你此刻就是 {{char}}，在给自己填一份「阅读偏好」档案，之后陪 {{user}} 读书、挑书会用上。\n' +
      '\n' +
      '按下面的 JSON 回（除了 JSON 不要写别的）：\n\n' +
      '{\n' +
      '  "genres": ["你偏爱的类型与流派，3-8 个"],\n' +
      '  "authors": ["你会反复读的作者"],\n' +
      '  "taste": { "be": 0, "he": 0, "gl": 0, "bg": 0 },\n' +
      '  "logic": 0,\n' +
      '  "appreciation": "你对不同文本的鉴赏方向，一两句",\n' +
      '  "scores": { "学术": 0, "文艺": 0, "美感": 0, "情感": 0, "互文": 0 },\n' +
      '  "penColor": "#d97f5a"\n' +
      '}\n\n' +
      '按你自己来填：\n' +
      '- 你是杂食的人：某个方向读得多，别的方向也有过被打动的时候。\n' +
      '- 但你有偏向，不会什么都爱、什么都懂。\n' +
      '- taste 四项各 0-10：你对 BE / HE / GL / BG 的偏爱程度。\n' +
      '- logic：0 = 你全凭感性，10 = 你全凭逻辑。\n' +
      '- scores 五项各 0-10：学术（会去查、会去推）、文艺（语感和留白）、美感（画面和意象）、情感（人的处境和情绪）、互文（会联想到别的书别的事）。\n' +
      '- penColor：你做笔记那支笔的颜色（十六进制 #rrggbb；上面那个只是格式示例，你自己挑）。挑一支**换个人就不会挑的**——你的笔色摆在那儿，别人一眼就知道是你的。**明暗浓淡都不限**，亮的、淡的、艳的、深的、灰的都行，贴你就好。唯一一条：纯黑纯白不是你会选的，那是没挑过的样子。',
  },
  {
    category: '读书',
    label: '阅读风格·气质',
    description: '第一次读书前取一次「阅读气质」：六个元素（两个动词 + 两个经典公式/数列 + 两个名词意象），和阅读本身无关，只作气质基调参考',
    // 她 09-20：这一份是**气质基调参考**，答案**不许和阅读本身有关联**（不是爱好，是人身上那股劲儿）。
    defaultValue:
      '你在给 {{char}} 取一份「阅读气质」。它不是爱好，**和读什么书无关**——是这个人做事、感受、说话的那股劲儿。\n' +
      '之后它只会作为**气质基调参考**出现（不是要求，也不是设定）。\n' +
      '\n' +
      '按下面的 JSON 回（除了 JSON 不要写别的）：\n\n' +
      '{\n' +
      '  "verbs": ["动词一", "动词二"],\n' +
      '  "forms": ["经典物理公式或经典函数/数列一", "二"],\n' +
      '  "images": ["名词意象一", "名词意象二"]\n' +
      '}\n\n' +
      '怎么取（就六个元素，多一个都不要）：\n' +
      '- **两个动词**：他没说话的时候，身上正在发生的那股劲儿。\n' +
      '- **两个公式或数列**：经典物理学的公式，或者经典的函数、数列。\n' +
      '- **两个名词意象**：和他本人接近、**但彼此之间毫无关联**的超具体、超感官化的真实意象；每个**不超过五个字**。\n' +
      '- 三类之间**越没关系越好**：自由组合，谁也别解释谁、别呼应谁。\n' +
      '- 意象要具体到能看见、摸到、闻到的程度，别用抽象名词。\n' +
      '- 整份答案**和读书、书、文字、故事都没有关系**——它是气质，不是爱好。',
  },
  {
    category: '读书',
    label: '共读·总结',
    description: '把这段时间一起读的、聊的记成一条第三人称流水（按归档规则自动写，共读结束时进聊天）',
    // **只写事儿，不碰角色信息**（她 09-16）：喂进来的只有「读了什么 / 聊了什么 / 他当时的心内话」，
    // 没有角色人设、没有 user 设定——推断心情、替谁抒情都不是它的活儿。
    // 提示词**一律用正面说法**（她 09-16 定的：总结模型这段不准写负面提示词）。
    // 她 09-20 追加：**写清楚从哪读到哪、整场共读持续了多久、有几个人就写几个人**。
    defaultValue:
      '你在替 {{user}} 和 {{char}} 记这段共读的流水账，留给他们以后回看。\n' +
      '\n' +
      '开头先写清楚这三样：\n' +
      '- 这一次是从哪儿读到哪儿（章和段，材料里给了就照抄）；\n' +
      '- 这一场共读一共持续了多久（材料里给了就照抄）；\n' +
      '- 一起读的是谁——**材料里有几个人就写几个人**，一个不漏，也别多写。\n' +
      '\n' +
      '然后按原样记这段时间发生的事，用第三人称（写名字，或者「他」「她」）：\n' +
      '- 他们读到的这几页大概讲了什么事；\n' +
      '- 每个人在书上说了什么，谁说的都写清楚；\n' +
      '- 他们读的时候心里冒出来的那些念头——照本来的说法记，各一两句。\n' +
      '\n' +
      '一条三四句话，250 字以内，直接写正文（纯文字，标题和格式标记都省掉）。\n' +
      '给的材料里有的就写，材料里没提的留空着。',
  },
];
...
/** 改过名的提示词：老键上的用户改动搬到新键（她 09-20：「读一页」有歧义，改成「读书」）。 */
const RENAMED_LABELS: Record<string, string> = { '共读·读一页': '共读·读书' };
(() => {
  try {
    ...
    let dirty = false;
    for (const [from, to] of Object.entries(RENAMED_LABELS)) {
      for (const suffix of ['', '▸rp']) {
        if (`${from}${suffix}` in all) {
          all[`${to}${suffix}`] = all[`${from}${suffix}`];
          delete all[`${from}${suffix}`];
          dirty = true;
        }
      }
    }
    if (dirty) localStorage.setItem(KEY, JSON.stringify(all));
  } catch { /* 读不出来就用默认值，不影响 */ }
})();
...
/** 两套的存储键：默认套就是 label 本身，rp 套加后缀（老数据不受影响）。 */
export const promptKey = (label: string, preset = ''): string => (preset ? `${label}▸${preset}` : label);

export const getPrompt = (label: string, preset = ''): string => {
  const entry = PROMPT_ENTRIES.find((e) => e.label === label);
  if (!entry) return '';
  const own = loadOverrides()[promptKey(label, preset)];
  if (typeof own === 'string') return own;
  return (preset && entry.rpValue) ? entry.rpValue : entry.defaultValue;
};

export const savePrompt = (label: string, content: string, preset = '') => {
  const all = loadOverrides();
  all[promptKey(label, preset)] = content;
  try {
