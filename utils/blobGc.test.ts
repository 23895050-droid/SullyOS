...
describe('引用面清单拼写守卫', () => {
    ...
    it('fork 专属表 image_receipts 必须留在引用面清单里', () => {
        // 相机/相册「近期接收」的原图存在这张表里。上游清单没有它，追上游时最容易
        // 被整表覆盖丢掉——一旦丢，GC 会把相机全部原图当孤儿删掉（不可逆）。钉死。
        expect(REF_SOURCE_STORES).toContain('image_receipts');
    });
});
