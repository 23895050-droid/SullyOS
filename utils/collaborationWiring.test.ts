...
describe('collaboration sidecar wiring', () => {
  ...
  it('opens every collaboration file through the shared forced-share pipeline', () => {
    const windowSource = read('features/collaboration/CollaborationWindow.tsx');
    // Settings 的 shareOrDownloadBlob 化是 fork 点之后上游的独立改动（不在本次系列内），
    // 这里只守协作窗口自己不另造下载轮子。
  ...
  it('refreshes task-related memory on every collaboration send without mixing windows', () => {
    ...
    // fork 版记忆宫殿管线还没有 entryPoint / formatter 参数：走标准 injectMemoryPalace 召回，
    // 模式相关的条数上限由本地词法打分兜底承担。
    expect(context).toContain('injectMemoryPalace(');
    expect(context).not.toContain("entryPoint: 'collaboration'");
    expect(context).toContain("mode === 'focused' ? 5 : 15");
