...
import { isBlobRef, getBlobForRef, restoreBlobRef, migrateDataUrlToRef, migrateAppearancePresetBlobRefs, migrateChatThemeBlobRefs, resolveBlobRefsDeep, BLOBREF_PREFIX, deleteBlobRefIfUnreferenced } from '../utils/blobRef';
      ...
      try {
        ...
        if (!finalChars.some(c => c.id === sullyV2.id)) {
            ...
            if (existingSully) {
                 ...
                 if (isCorrupted || !existingSully.roomConfig || needsWallUpdate || needsSkinSets || needsAvatarUpdate) {
  ...
  const exportSystem = async (mode: 'text_only' | 'media_only' | 'full'): Promise<Blob> => {
      try {
          ...
          for (const storeName of storesToProcess) {
              ...
              // src/shadowSrc……）不在这里做任何处理：v3 令牌原样进 JSON，onSerialized
              // 统一收集、二进制随 blobs/* 旁路走，任何 store 的令牌都覆盖，没有名单可漏。
