...
const memoryScore = (node: MemoryNode, queryTokens: Set<string>, now: number): number => {
  // fork 版 MemoryNode 没有 entities 字段（实体提取是 feature-batch 的功能，未搬运），
  // 召回打分只看正文 + 标签。
...
export const buildCollaborationContextSnapshot = async ({
  ...
  return [
    // fork 版 buildCoreContext 没有 conversational / timeOptions 参数（feature-batch 才加的），
    // 沉浸式协同要求「与 ChatApp 完全同一套管线」，直接走默认参数即可。
    ContextBuilder.buildCoreContext(staticChar, user, true),
...
export const buildCollaborationTurnMemoryContext = async (input: {
  ...
  try {
    // fork 版内存宫殿管线还没有 entryPoint / formatterMaxOutputItems 参数，
    // 走标准召回即可（召回为空时下方还有本地词法打分兜底）。
